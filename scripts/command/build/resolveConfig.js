module.exports = (api, args, options) => {
  ...
  if (args.modern) {
    const ModernModePlugin = require('../../../config/webpack/ModernModePlugin');
    if (!args.modernBuild) {
      // Inject plugin to extract build stats and write to disk
      config.plugin('modern-mode-legacy').use(ModernModePlugin, [
        {
          targetDir,
          isModernBuild: false,
      ...
      config.plugin('modern-mode-modern').use(ModernModePlugin, [
        {
          targetDir,
          isModernBuild: true,
          unsafeInline: args['unsafe-inline'],
